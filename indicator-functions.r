...
calc_domain_scores <- function(d,
  ...
  if (bespoke.domains) {
    ...
    d <- d %>%
      ...
      # combine with equal weights to get domain score
      mutate(`Vulnerability score` = sum(
        `Clinical Vulnerability rank_scaled` * clinical_weight,
...
aggregate_scores <- function(d, domain = NULL, aggregate_by, population_col = "No. people", max_rank = NULL) {
  ...
  d_scores <- d %>%
    ...
    group_by(!!sym(aggregate_by)) %>%
    summarise(
      Extent = sum(Extent) / sum(!!sym(population_col)),
...
calc_vaccine_scores <-
  function(data) {
    ...
    # (to mirror regression standardised Beta coefficients)
    data <- data %>%
      mutate_at(vars(ends_with("rank")), list(scaled = scale_ranks))
